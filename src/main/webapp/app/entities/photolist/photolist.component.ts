...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/photolist'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/photolist', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: Photolist) {
        return item.id;
    }
    registerChangeInPhotolists() {
        this.eventSubscriber = this.eventManager.subscribe('photolistListModification', (response) => this.loadAll());
    }
